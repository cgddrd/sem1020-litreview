...
%For them to operate safely within unconstrained, unstructured and often unforgiving dynamic environments, autonomous vehicles depend upon an unprecedented degree of inductive inference and high-level reasoning - two factors guaranteed to increase software complexity, and render many traditional software   verification techniques inadequate to the task of assessing  .
...
%The application of high-level autonomy is of course not just limited to driverless vehicles. Military drones, exploratory research robots and now even ``connected" household thermostats all rely upon some degree of autonomy in order to operate independently of human control.
...
%In order to drive the assessment of an autonomous system against its required behavioural properties, the authors employ 
%
%
%Formal verification describes the process of employing mathematical and logical methods to evaluate the behaviour of algorithm against the expectations stipulated as part of a pre-established specification \cite{fisher-2}. 
%
%%
%%In order for this type of evaluation to become possible, the authors propose to exploit existing methods of formal verification for rational-agents in order
%
%
%Fisher et al. take a wider view toward the evaluation of autonomous systems, discussing their proposal to apply formal verification within the context of a general autonomous system architecture, as opposed to focussing upon any specific field or application \cite{fisher-1}. 
%
% Fisher et al. propose to extend this technique by employing the \textit{Belief-Desire-Intention} (BDI) rational-agent architecture as the basis for establishing a model of system behaviour used to support upcoming verification within . The BDI

% incorporate existing rational-agent logic as the basis for this behaviour specification \cite{fisher-1}.
...
%
%
%Whilst it seems insurmountable for testing and verification to ever eradicate the possibility of an autonomous system undertaking an unsafe action, it is reassuring to think that methods capable of confirming such actions were not taken in preference over a safe
%
%
%
%
...
%
%
%Whilst limitations of existing validation practices are likely to constrain the scope of functionality exhibited by autonomous vehicles \cite{wagner-2}, over time these constraints may become 
%
%
%
%
...
%What perhaps is most striking 

%
%
%What is most striking about these two papers, is that they appear to offer entirely divergent views on what should become recommended best practice for affording extensive and . In the first paper 
%
%
%
%
%
% the clear divergence they exhibit in they views for to addressing the same fundamental problem of advancing testing practices t
%



%
%The first challenge refers to the fact that human drivers can no longer be relied upon to relieve an autonomous control system in the event of exceptional circumstances. This naturally raises issues over the fallbacks such a system must feature to handle these types of situations itself, and the inevitable increase in software complexity this brings as a consequence. The second challenge refers to the general incompatibility placed between the expectation for software verification to be based upon fixed and unambiguous requirements, and the fact that autonomous control systems by their very nature are expected work to under an incredibly large and diverse set of conditions that could not feasibly be represented using a formal requirements specification. 
...
% one must gather an infinite amount of evidence \textit{conclusively} prove a theory hold, but it only requires a single negative observation to conclusively refute the theory. 
% 
% 
% to conclusively prove a theory one must gather an infinite amount of supporting evidence, however to disprove a theory, one must find 
% 
% that a theory can never be conclusively proven through the use of induction \cite{problem-of-induction}, but \textit{can} be conclusively disproved by 
% 
% 
% the use of inductive logic for building a theory 
...
%As cyber-criminals begin to show increasing attention toward the exploitation so-called ``connected" vehicles, manufacturers must find new ways to sufficiently demonstrate that their systems can deliver a robust defence against malicious attacks that risk compromising vehicle safety. \cite{yagdereli}.
